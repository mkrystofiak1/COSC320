...
bool Matrix::needs_Padding() const
{
	...
	if (ceil(log2(rows)) == floor(log2(rows)))
		if (ceil(log2(columns)) == floor(log2(columns)))
			return false;
	return true;
}
...
{	
	if (this->rows == 1)
	 ...
	 {	 
		 Matrix temp = this->pad();
		 unsigned long halfn = temp.rows/2;
		 ...
		 for (int i = 0; i < temp.rows; i++)
			 ...
			 {
				 if (i < halfn && j < halfn)
					B.arr[i][j] = temp.arr[i][j];
				 else if ( i < halfn && j >= halfn)
					Ct.arr[i][j-halfn] = temp.arr[i][j];
				 else if (i >= halfn && j < halfn)
					C.arr[i-halfn][j] = temp.arr[i][j];
				 else if (i >= halfn && j >= halfn)
					D.arr[i-halfn][j-halfn] = temp.arr[i][j];
			 }
		 ...
		 Matrix Ainv(temp.rows, temp.rows);
		 ...
		 Matrix resize(this->rows, this->columns);
		 for (int i = 0; i < this->rows; i++)
			 for (int j = 0; j < this->columns; j++)
				 resize.arr[i][j] = Ainv.arr[i][j];
		 return resize;
	 }
	 else
	 {
		 return ((*this*this->transpose()).inverse())*this->transpose();
	 }
}
